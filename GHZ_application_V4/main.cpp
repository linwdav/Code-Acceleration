...
int main(int argc, char *argv[])
{
    ...
    for (int l=1; l<count_xy_glbl+1; l++)
    {

            
            star = new star_cell [column_array[l]->num_stars+1];
            //loop that iterates through each cell in the column
            for (long int j=column_array[l]->cell_min; j<(column_array[l]->cell_max)+1; j++)
            {
                
                    for (int k=1; k<(subsect_ptr[j]->num_stars)+1; k++)
                    {
...
//} //end of omp pragma
         ...
         for (long int l=1; l<ptr_2_range+1; l++)
         {
                  ...
                  if(star2[l].sn==1 || star2[l].sn==2)
                  {
                       ...
                       sn_z=star2[l].z_coord;  
                           ...
                           for (long int m=get_star_range_lower_cell_left(star2[l].cell); m<star_range_upper_cell_left+1; m++)
                              {
                                  normal_ster_flag=0;
                                  wrap_ster_flag=0;    
...
int wrap_around_function(float x, float y, float z, float sterilization_distance)
{
    ...
    float temp2 = PI_fn/180;
    
    R=sqrt((x*x)+(y*y));
    
    angle=(acos(x/R))*(180/PI_fn);
    
    float temp = (PI_fn/360)*2;

    arc_length_original=(angle*temp)*(R);
    arc_length_total=(mw_angle_fn*temp)*(R); 
    ...
    if (arc_length_ratio<=0.5)
    {
        ...
        wrap_around.x_coord=R*(cos(outside_angle*temp2));                
        wrap_around.y_coord=R*(sin(outside_angle*temp2)); 
    ...
    else //(arc_length_ratio>0.5)
    {
        ...
        wrap_around.y_coord=-1*(R*(sin(outside_angle*temp2)));    
        wrap_around.z_coord=z;        
         return 1;   
    }    
...
long int get_star_range_lower_cell_left(int cell)
{
    ...
    long int total=0;
    int column = (subsect_ptr[cell]->subsection_xy)-1;
    ...
    {
    stellar_index=subsect_ptr[cell-count_z_glbl]->star_index_min-(total); 
    }
    else
    {
    stellar_index=subsect_ptr[cell-count_z_glbl-1]->star_index_min-(total);    
    }
...
long int get_star_range_upper_cell_left(int cell)
{
    ...
    {
    stellar_index=subsect_ptr[cell-count_z_glbl]->star_index_max-(total);
    }
    else
    {
    stellar_index=subsect_ptr[cell-count_z_glbl+1]->star_index_max-(total);    
    }
...
long int get_star_range_upper(int sn_star)
{
    ...
    else if (subsection==count_z_glbl)
    {
         ...
         upper=subsect_ptr[cell]->star_index_max-(total);               
    }
    else
    {
         long int total=0;
         ...
         upper=subsect_ptr[cell+1]->star_index_max-(total); 
    }
...
void print_stats()
{
     ...
     for (int i=1; i<(column_array[stats_counter]->num_stars)+1; i++)
     {
